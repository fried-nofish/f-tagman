...
void FuncWindow::init_btn()
{
    ...
    connect(btn_renametag,&QPushButton::clicked,this,&FuncWindow::on_pushButton_5_clicked);
    connect(btn_openfile,&QPushButton::clicked,this,&FuncWindow::on_pushButton_6_clicked);
...
void FuncWindow::init_title()
{
    this->setWindowIcon(QIcon(":/img/logo.jpg"));
        ...
        connect(btn_off_screen, &QPushButton::clicked, this, &FuncWindow::showMinimized);
...
void FuncWindow::on_pushButton_5_clicked()//标签改名
{
    ...
    if(result){
        ...
        if(bOk)tagrename(taglist[index].name, sName.toStdString(), "");  //标签改名
        ui->tableWidget->setRowCount(0);
        int row_num = 0;
        for(auto &i : taglist[index].T_filelist){
...
{
    if(ui->tableWidget ->item(row, col)->checkState() == Qt::Checked && col != 0){
        open_col = col;
        open_row = row;
...
void FuncWindow::on_pushButton_6_clicked()
{
    ...
    QString FileShellCommand = dir.absoluteFilePath(str); //qstring_filename 文件名
