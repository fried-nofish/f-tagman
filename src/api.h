...
struct Tag {
    ...
    bool operator==(const Tag &T) {
        return (this->name == T.name) && (this->explain == T.explain)
            && (this->T_filelist == T.T_filelist);
    }
};
...
File fileinset(
    string name, string addr); // 确认输入文件存在于vector中，用于给文件加标签
std::pair<File, Tagint> fileinvec(
    string name, string addr); // 确认输入文件存在于vector中，用于删标签
...
bool tagexplain(
    string name, string old_explain, string new_explain); // 给标签添加说明
