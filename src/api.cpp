...
bool fileindir(File file) {
    ...
    if (fs::exists(s) && fs::is_directory(s)) { return true; }
    return false;
} // 确认输入文件存在于目录下
...
std::pair<File, Tagint>
    fileinvec(string filename, string addr, string tagname, string explain) {
    for (int i = 0; i < TagList.size(); ++i) {
        for (int j = 0; j < TagList[i].T_filelist.size(); ++j) {
            if (TagList[i].T_filelist[j].name == filename
                && TagList[i].T_filelist[j].address == addr
...
bool filedeltag(File file, Tagint tagpoint) {
    for (int i = 0; i < TagList[tagpoint].T_filelist.size(); i++) {
        ...
        if (TagList[tagpoint].T_filelist[i].name == file.name
            && TagList[tagpoint].T_filelist[i].address == file.address) {
            TagList[tagpoint].T_filelist.erase(
                TagList[tagpoint].T_filelist.begin() + i);
            return 1;
        }
    }
    return 0;
...
std::vector<File> showallfile() {
    std::vector<File> a_file;

    for (auto &it : FileList) { a_file.push_back(it); }
    return a_file;
}
